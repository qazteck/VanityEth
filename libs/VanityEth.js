...
var isValidVanityWallet = function (wallet, input, isChecksum, isContract, hasEnd, end) {
    ...
    if (isContract) {
        ...
        return _contractAdd.substr(2, input.length) == input
            && (hasEnd == 'false' || _contractAdd.substr(_contractAdd.length - end.length, _contractAdd.length) == end)
    }
    _add = isChecksum ? ethUtils.toChecksumAddress(_add) : _add;
    return _add.substr(2, input.length) == input
        && (hasEnd == 'false' || _add.substr(_add.length - end.length, _add.length) == end);
}
var getVanityWallet = function (input, isChecksum, isContract, hasEnd, end) {
    if (!isValidHex(input) || (hasEnd == 'true' && !isValidHex(end))) throw new Error(ERRORS.invalidHex);
    input = isChecksum ? input : input.toLowerCase();
    var _wallet = getRandomWallet();
    while (!isValidVanityWallet(_wallet, input, isChecksum, isContract, hasEnd, end)) _wallet = getRandomWallet(isChecksum);
